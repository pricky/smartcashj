...
import cc.smartcash.smartcashj.utils.ContextPropagatingThreadFactory;
 ...
 * smartcashj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
     ...
     * independently use smartcashj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a smartcashj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using smartcashj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing smartcashj via a thread that has not had any context set on it.");
