...
/**
 * A tool for comparing the blocks which are accepted/rejected by bitcoind/smartcashj
 * It is designed to run as a testnet-in-a-box network between a single bitcoind node and smartcashj
        ...
        for (Rule rule : blockList.list) {
            if (rule instanceof FullBlockTestGenerator.BlockAndValidity) {
                ...
                if (!chain.getChainHead().getHeader().getHash().equals(bitcoindChainHead)) {
                    rulesSinceFirstFail++;
                    log.error("ERROR: bitcoind and smartcashj acceptance differs on block \"" + block.ruleName + "\"");
                }
                if (block.sendOnce)
